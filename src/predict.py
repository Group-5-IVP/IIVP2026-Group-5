...
from dataset import test_csv_path, train_img_folder_path, test_img_folder_path, DigitDataset
...
def predict_ensemble(models:list, test_df, imgs_path, use_tta=False, batch_size=128):
    all_df_probs = []
    for model in models:
        pred = predict(model, imgs_path, test_df, use_tta, batch_size)
...
def predict(model, imgs_path, test_df=None, use_tta=False, batch_size=128):
